...
### AOP-103 — Найти и продолжить произвольную локальную задачу

Добавить ограниченный поиск по локальной state DB Codex без сканирования
истории и отправку сообщения в существующую задачу по `threadId`. Сценарий
должен работать для задач, которые не сопоставлены опубликованному проекту.
Coordinator и Mac-worker работают на `0.1.4`; Mac E2E успешно нашёл свежую
задачу и продолжил её по ID. Для Windows E2E требуется обновление worker до
`0.1.4`.
...
- [ ] **AOP-070:** Передать Git-файл Mac → Windows. Windows-worker обновлён до
  `0.1.3`; требуется повторить реальный сценарий в этом направлении после
  AOP-103.
