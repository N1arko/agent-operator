...
установлен, прошёл диагностику и отправлял heartbeat до `20:47:58Z`.
Windows-ноутбук отключился после разрядки аккумулятора. После следующего
включения требуется вручную запустить worker и повторить контрольный ответ
`ready`.
...
**Статус:** worker `0.1.2` установлен; ноутбук offline после разрядки,
ожидается финальный `agent_start → agent_wait`.
