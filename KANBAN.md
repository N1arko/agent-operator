...
Входные критерии выполнены. Подготовленная задача находится в
`docs/CHECKPOINT_WINDOWS.md`. Требуется включить Windows-ноутбук, запустить
Windows Codex и передать ему задачу.

## Контрольные точки

### CP-WIN-01 — Подключить реальный Windows-ноутбук

**Статус:** готов к действиям пользователя.
...
- [ ] Windows-worker присылает heartbeat.
- [ ] `agents_list` показывает Windows-agent как `idle`.
- [ ] `agent_projects` возвращает локальные Windows-проекты.
- [ ] Тестовый `agent_start` запускает свежий thread в выбранном проекте.
- [ ] Результат возвращается через `agent_wait`.
- [ ] Перезапуск Windows-worker восстанавливает подключение.
