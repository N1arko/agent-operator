...
- локальный вертикальный тест и реальный удалённый `agent_start → agent_wait`
  проходят;
- Windows-worker `0.1.1` подключён, публикует проект и запускает свежий Codex
  thread;
- worker `0.1.2` исправляет возврат финального текста Codex `0.145.0` и
  подготовлен для контрольного обновления Windows.
