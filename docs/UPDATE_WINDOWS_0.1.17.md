# Обновление Windows-worker до 0.1.17

## Цель

Обновить worker и проверить жизненный цикл первого удалённого Desktop-turn.
Follower IPC-соединение должно оставаться открытым до терминального результата,
чтобы prompt, прогресс и ответ сразу отображались в новой карточке Codex
Desktop.
...
- Scheduled Task запускает каталог `0.1.17`;
- heartbeat показывает worker `0.1.17`;
...
3. Распаковать package в
   `C:\Users\nikit\AppData\Local\AgentOperator\0.1.17`.
...
7. Обновить action Scheduled Task `Agent Operator Worker` на
   `0.1.17\run-worker.ps1`, сохранив trigger, principal и settings.
...
Каталог `0.1.16` сохраняется для отката.
